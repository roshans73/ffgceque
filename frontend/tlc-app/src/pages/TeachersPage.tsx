...
const EMPTY_FORM: TeacherForm = {
  name: '', school: '', districtId: '', blockId: '',
  gender: '', mobile: '', email: '',
  isTipTeacher: false, yearsInTip: '', coachId: '',
};
...
const TeachersPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    apiClient.getCoaches().then((r) => setCoaches(r.data)).catch(console.error);
    loadTeachers();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      await apiClient.createTeacher({
        ...
        yearsInTip:   form.isTipTeacher && form.yearsInTip ? Number(form.yearsInTip) : null,
  ...
  return (
    <Box>
            ...
            <TableBody>
              {filtered.map((t) => (
                <TableRow key={t.id} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                  <TableCell sx={{ fontFamily: 'monospace', fontSize: '0.85rem' }}>{t.teacherCode}</TableCell>
                  <TableCell sx={{ fontWeight: 500 }}>{t.name}</TableCell>
                  <TableCell sx={{ color: 'text.secondary' }}>{t.school}</TableCell>
                  <TableCell>{t.gender}</TableCell>
                  <TableCell sx={{ color: 'text.secondary' }}>{t.mobile}</TableCell>
                  <TableCell>
                    {t.isTipTeacher && <Chip label="TIP" color="primary" size="small" />}
                  </TableCell>
                </TableRow>
              ))}
              {filtered.length === 0 && (
      ...
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          <Typography variant="h6" sx={{ fontWeight: 700 }}>Add Teacher</Typography>
        </DialogTitle>
        <Divider />
        <DialogContent sx={{ pt: 2.5 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <TextField label="Full Name" value={form.name} required
              onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} fullWidth />
            <TextField label="School" value={form.school}
              onChange={(e) => setForm((p) => ({ ...p, school: e.target.value }))} fullWidth />
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <FormControl sx={{ width: 120 }}>
                ...
                <Select value={form.gender} label="Gender"
                  onChange={(e: SelectChangeEvent) => setForm((p) => ({ ...p, gender: e.target.value }))}>
                  <MenuItem value="M">Male</MenuItem>
                  <MenuItem value="F">Female</MenuItem>
                </Select>
              </FormControl>
              <TextField label="Mobile" value={form.mobile} sx={{ flex: 1 }}
                onChange={(e) => setForm((p) => ({ ...p, mobile: e.target.value }))} />
              <TextField label="Email" type="email" value={form.email} sx={{ flex: 1 }}
                onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} />
            </Box>

            <FormControl fullWidth>
              <InputLabel>Coach (optional)</InputLabel>
              <Select value={form.coachId} label="Coach (optional)"
                onChange={(e: SelectChangeEvent) => setForm((p) => ({ ...p, coachId: e.target.value }))}>
                <MenuItem value=""><em>None</em></MenuItem>
                {coaches.map((c) => (
                  <MenuItem key={c.id} value={String(c.id)}>{c.name} ({c.empNo})</MenuItem>
                ))}
              </Select>
            </FormControl>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, isTipTeacher: e.target.checked, yearsInTip: '' }))}
                  />
                }
                label="TIP Teacher"
              />
              {form.isTipTeacher && (
                <TextField
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, yearsInTip: e.target.value }))}
                  sx={{ width: 140 }}
